/* Harnais de validation — Capitals de Washington (jsdom) */
const fs = require('fs');
...
const ATTENDU = {
  equipe: 'WASHINGTON',
  taille: 23,
  echantillon: {nom: 'Andrey Vasilevskiy', salaire: 11575000, ct: 1, ov: 85},
  masseSousContrat: 60700000,
  nbSousContrat: 10
};
...
(async () => {
  ...
  const vHei = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Danton Heinen'), 'UFA', 1);
  egal(vHei.code, 'surpaye', 'Danton Heinen (OV 81, 31 ans, 9,075 M, UFA) → Surpayé');
  egal(vHei.minEff, 6250000, 'Charte Heinen : 6 250 000 $');
  egal(vHei.ecart, 2825000, 'Écart Heinen : +2 825 000 $');
  const vMai = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Logan Mailloux'), 'RFA', 1);
  egal(vMai.code, 'aubaine', 'Logan Mailloux (OV 77, 23 ans, 900 k, RFA) → Aubaine salariale');
  egal(vMai.ecart, 50000, 'Écart Mailloux : −50 000 $ sous la charte (950 k)');
  ...
  const vPri = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Cayden Primeau'), 'RFA', 1);
  egal(vPri.minEff, 950000, 'Cayden Primeau (G, OV 78, 27 ans, RFA) évalué à l\'échelon 77 : 950 000 $');
  egal(vPri.code, 'surpaye', 'Cayden Primeau à 1,075 M → Surpayé');
  ...
  selJoueur.value = 'Danton Heinen';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('surpaye'), 'Heinen sélectionné : badge Surpayé');
  ...
  egal(celluleActive.textContent.replace(/[\s\u00a0\u202f]/g,''), '6250$',
    'Cellule active : 6 250 $ (UFA 34-, OV 81, Heinen)');
  ...
  selJoueur.value = 'Cayden Primeau';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('surpaye'), 'Cayden Primeau : badge Surpayé');
